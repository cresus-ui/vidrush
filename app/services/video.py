...
from app.services import subtitle, transitions

# Create a local alias to legacy 'utils' name if needed
utils = app_utils
...
def get_ffmpeg_binary():
    ...
    # Use imageio_ffmpeg to get the bundled modern FFmpeg binary by default
    try:
        import imageio_ffmpeg
        return os.environ.get("IMAGEIO_FFMPEG_EXE") or imageio_ffmpeg.get_ffmpeg_exe()
    except Exception:
...
def combine_videos_with_transitions(
    ...
    """
    Concatenate videos using FFmpeg xfade transitions.
    ...
    if len(video_paths) == 1:
        # Simple re-encode for single video
        cmd = [
            get_ffmpeg_binary(), "-y", "-i", video_paths[0],
    ...
    # Get actual durations for precise xfade offsets
    clip_durations = []
    ...
    if len(video_paths) == 2:
        offset = max(0, clip_durations[0] - trans_duration)
        trans = transitions.get_random_transition(category)
        filter_parts.append(
            f"[n0][n1]xfade=transition={trans}:duration={trans_duration}:offset={offset}[outv]"
        )
    else:
        # First xfade: [n0][n1] -> [v0]
        offset = max(0, clip_durations[0] - trans_duration)
        trans = transitions.get_random_transition(category)
        filter_parts.append(
            f"[n0][n1]xfade=transition={trans}:duration={trans_duration}:offset={offset}[v0]"
        )

        # Subsequent xfades
        for i in range(2, len(video_paths)):
            offset += max(0, clip_durations[i - 1] - trans_duration)
            trans = transitions.get_random_transition(category)

            # For news, occasionally use pixelize for glitch feel
            if category == "news" and random.random() > 0.5:
                trans = "pixelize"

            prev_label = f"v{i - 2}"
            if i == len(video_paths) - 1:
                # Last xfade outputs to [outv]
                filter_parts.append(
                    f"[{prev_label}][n{i}]xfade=transition={trans}:duration={trans_duration}:offset={offset}[outv]"
                )
            else:
                filter_parts.append(
                    f"[{prev_label}][n{i}]xfade=transition={trans}:duration={trans_duration}:offset={offset}[v{i - 1}]"
                )
    ...
    logger.info(f"Executing FFmpeg xfade command for {len(video_paths)} clips (category: {category})")
    ...
    xfade_success = (
        ...
        and os.path.getsize(combined_video_path) > 1000
    )

    if not xfade_success:
        stderr_snippet = (result.stderr or "")[-800:]
        logger.warning(f"FFmpeg xfade failed (code={result.returncode}), falling back to simple concat.\nStderr: {stderr_snippet}")
        # Remove any corrupted partial file
        if os.path.exists(combined_video_path):
            os.remove(combined_video_path)
        # Fallback to simple concat
        output_dir = os.path.dirname(combined_video_path)
        concat_video_clips_with_ffmpeg(video_paths, combined_video_path, threads, output_dir)

    # Final validation
    if not os.path.exists(combined_video_path) or os.path.getsize(combined_video_path) < 1000:
        raise RuntimeError(
            f"Failed to produce valid combined video at {combined_video_path}. "
            f"Both xfade and concat fallback failed. Check FFmpeg installation and input files."
        )
    logger.info(f"Combined video created successfully: {combined_video_path} ({os.path.getsize(combined_video_path)} bytes)")
...
def generate_video(
    ...
    # Add SFX
    if sfx_materials:
        logger.info(f"  ⑥ adding {len(sfx_materials)} SFX clips")
        for i, sfx in enumerate(sfx_materials):
            try:
                sfx_path = sfx["path"]
                if os.path.exists(sfx_path):
                    sfx_clip = AudioFileClip(sfx_path).with_effects([afx.MultiplyVolume(0.5)])
                    # Distribute SFX every 10 seconds or based on number of clips
                    start_time = i * (video_clip.duration / (len(sfx_materials) + 1))
                    if start_time < video_clip.duration:
                        sfx_clips = sfx_clip.with_start(start_time)
                        audio_clips.append(sfx_clips)
            except Exception as e:
                logger.error(f"failed to add sfx {i}: {str(e)}")
...
def preprocess_video(materials: List[MaterialInfo], clip_duration=4, category="finance"):
    ...
    for material in materials:
        ...
        try:
            ...
            if ext in const.FILE_TYPE_IMAGES:
                ...
                zoom_speed = transitions.get_zoom_speed(category)
                zoom_clip = clip.resized(
                    lambda t: 1 + (clip_duration * zoom_speed) * (t / clip.duration)
                )
