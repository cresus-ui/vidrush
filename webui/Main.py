...
if not config.app.get("hide_config", False):
    with st.expander(tr("Basic Settings"), expanded=False):
        ...
        with right_config_panel:
            ...
            st.write(tr("Upgrade Settings"))
            freesound_api_key = config.app.get("freesound_api_key", "")
            freesound_api_key = st.text_input(
                "Freesound API Key", value=freesound_api_key, type="password"
            )
            config.app["freesound_api_key"] = freesound_api_key

            wiki_enabled = st.checkbox(
                "Enable Wikipedia Person Visuals", value=config.app.get("wiki_enabled", True)
            )
            config.app["wiki_enabled"] = wiki_enabled

            auto_sfx = st.checkbox(
                "Enable Auto Sound Effects (SFX)", value=config.app.get("auto_sfx", True)
            )
            config.app["auto_sfx"] = auto_sfx
...
with middle_panel:
    with st.container(border=True):
        ...
        video_transition_modes = [
            ...
            (tr("Intelligent (AI)"), VideoTransitionMode.intelligent.value),
